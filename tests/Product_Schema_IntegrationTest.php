...
/**
 * Class Product_Schema_IntegrationTest.
 *
 * @testdox Product Schema for a single post "download"
 */
class Product_Schema_IntegrationTest extends \WP_UnitTestCase {
	...
	private function get_post_content(): string {
		return <<<'EOL'
			...
			<!-- wp:yoast/faq-block {"questions":[{"id":"faq-question-1764935399988","question":["What WordPress plugins do I need?"],"answer":["Yoast SEO and Easy Digital Downloads."],"jsonQuestion":"What WordPress plugins do I need?","jsonAnswer":"Yoast SEO and Easy Digital Downloads."}]} -->
			<div class="schema-faq wp-block-yoast-faq-block"><div class="schema-faq-section" id="faq-question-1764935399988"><strong class="schema-faq-question">What WordPress plugins do I need?</strong> <p class="schema-faq-answer">Yoast SEO and Easy Digital Downloads.</p> </div> </div>
			<!-- /wp:yoast/faq-block -->
			EOL;
	}

	public function test_should_type_download_webpage_as_itempage(): void {
		...
		$this->assertArrayNotHasKey( 'dateModified', $webpage_piece, 'webpage should not be timestamped' );
	}
	
	public function test_should_leave_itempage_untouched(): void {
		$post_id = self::factory()->post->create(
			array(
